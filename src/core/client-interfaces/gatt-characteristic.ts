...
export class GattCharacteristic extends BaseInterface<GattCharacteristic1> {
    ...
    */
	...
	WriteAcquired = new ReadOnlyProperty<boolean>('WriteAcquired', this._internal)

	//@property({ name: 'NotifyAcquired', signature: 'b', access: ACCESS_READ })
	NotifyAcquired = new ReadOnlyProperty<boolean>('NotifyAcquired', this._internal)
	...
	async acquireWrite(options: dict<string, Variant>) { return this._internal.AcquireWrite(options) }

	//@method({ name: 'AcquireNotify', inSignature: 'a{sv}', outSignature: 'hq' })
	async acquireNotify(options: dict<string, Variant>) { return this._internal.AcquireNotify(options) }

	//@method({ name: 'StartNotify', inSignature: '', outSignature: '' })
	async startNotify() { return this._internal.StartNotify() }
