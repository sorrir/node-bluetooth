...
export class Device extends BaseInterface<Device1> {
    ...
    */
	...
	LegacyPairing = new ReadOnlyProperty<boolean>('LegacyPairing', this._internal)
	...
	ManufacturerData = new ReadOnlyProperty<dict<uint16, Variant>>('ManufacturerData', this._internal)

	//@property({ name: 'ServiceData', signature: 'a{sv}', access: ACCESS_READ })
	ServiceData = new ReadOnlyProperty<dict<string, Variant>>('ServiceData', this._internal)
	...
	ServicesResolved = new ReadOnlyProperty<boolean>('ServicesResolved', this._internal)

	//@method({ name: 'Disconnect', inSignature: '', outSignature: '' })
	async disconnect() { return this._internal.Disconnect() }

	//@method({ name: 'Connect', inSignature: '', outSignature: '' })
	async connect() { return this._internal.Connect() }

	//@method({ name: 'ConnectProfile', inSignature: 's', outSignature: '' })
	async connectProfile(UUID: string) { return this._internal.ConnectProfile(UUID) }

	//@method({ name: 'DisconnectProfile', inSignature: 's', outSignature: '' })
	async disconnectProfile(UUID: string) { return this._internal.DisconnectProfile(UUID) }

	//@method({ name: 'Pair', inSignature: '', outSignature: '' })
	async pair() { return this._internal.Pair() }

	//@method({ name: 'CancelPairing', inSignature: '', outSignature: '' })
	async cancelPairing() { return this._internal.CancelPairing() }
}