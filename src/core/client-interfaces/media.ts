...
export class Media extends BaseInterface<Media1> {
   ...
   */
  private constructor(bluez: Bluez, internal: Media1) {
  ...
  async registerEndpoint(endpoint: path, properties: dict<string, Variant>) {
    return this._internal.RegisterEndpoint(endpoint, properties);
  }

  //@method({ name: 'UnregisterEndpoint', inSignature: 'o', outSignature: '' })
  async unregisterEndpoint(endpoint: path) {
    return this._internal.UnregisterEndpoint(endpoint);
  }

  //@method({ name: 'RegisterPlayer', inSignature: 'oa{sv}', outSignature: '' })
  async registerPlayer(player: path, properties: dict<string, Variant>) {
    return this._internal.RegisterPlayer(player, properties);
  }

  //@method({ name: 'UnregisterPlayer', inSignature: 'o', outSignature: '' })
  async unregisterPlayer(player: path) {
    return this._internal.UnregisterPlayer(player);
  }
