...
export class DBusObjectManager {
	...
	readonly InterfacesAdded: Signal<{ path: path, objects: dict<string, dict<string, Variant>> }>
	readonly InterfacesRemoved: Signal<{ path: path, interfaceNames: Array<string> }>
	...
	*/
	...
	private constructor(path: path, internal: OrgfreedesktopDBusObjectManager) {
		...
		this.InterfacesAdded = new Signal<{ path: path, objects: dict<string, dict<string, Variant>> }>('InterfacesAdded',
			this._internal,
			{ path: null, objects: null },
			(event) => event,
			this._eventFilter)
		this.InterfacesRemoved = new Signal<{ path: path, interfaceNames: Array<string> }>('InterfacesRemoved', this._internal, { path: null, interfaceNames: null },
			(event) => event,
			this._eventFilter
		)
	}
	...
	branch(path: path): DBusObjectManager {
		if (!(path.startsWith(this.path))) {
			throw new Exception(`Cannot branch DBusObjectManager: The path '${path}' is not a subpath of '${this.path}'`)
		}
	...
	static async __connect(bluez: Bluez): Promise<DBusObjectManager> {
		if (bluez.objectManager !== undefined) {
			throw new Exception("Cannot connect to DBusObjectManager: The DBusObjectManager is already initialized")
		}
		return new DBusObjectManager("/", await OrgfreedesktopDBusObjectManager.Connect(bluez.bus, "/"))
	}
