...
export class DBusObject {
    ...
    async init() {
        ...
        if (this.hasProperties)
            this._properties = this._object.getInterface('org.freedesktop.DBus.Properties')
        return this
    }
    ...
    async get(parameterName: string) {
        ...
        let variant = await this._properties.Get(this.name, parameterName)
    ...
    async set(parameterName: string, value: dBusType, signature = null) {
        ...
        return this._properties.Set(this.name, parameterName, new Variant(signature, value))
    }
