...
export class DBusProperties {
	...
	*/
	...
	async getAllValues(): Promise<{ [K in string]: dBusType }> {
		let properties = {}
		for (let [name, variant] of Object.entries(<{ [K in string]: Variant }>(await this.getAll()))) {
    ...
    */
	...
	async get(name: string) { return this._internal.Get(this.interfaceName, name) }

	//@method({ name: 'Set', inSignature: 'ssv', outSignature: '' })
	async set(name: string, value: Variant) { return this._internal.Set(this.interfaceName, name, value) }

	//@method({ name: 'GetAll', inSignature: 's', outSignature: 'a{sv}' })
	async getAll() { return this._internal.GetAll(this.interfaceName) }
