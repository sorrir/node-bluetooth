...
export class HostGattCharacteristic extends BaseHostInterface {
    ...
    constructor(bluez: Bluez, service: HostGattService, uuid: string, flags: string[] = [], index: uint16 = 0) {
        super(bluez,
            HostGattCharacteristic.GetPath(service, index),
            `org.bluez.GattCharacteristic1`, {
            'Service': { signature: 'o', value: service, valueTransform: (val) => val.path },
            'UUID': new Variant('s', uuid),
            'Flags': new Variant('as', flags),
            'Descriptors': { signature: 'ao', value: {}, valueTransform: (val) => Object.keys(val) },
        })
        this._init()
    }
    ...
    addDescriptor(uuid: string, flags: string[] = [], index: uint16 = 0) {
        let descriptor = new HostGattDescriptor(this.bluez, this, uuid, flags, index)
